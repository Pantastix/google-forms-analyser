...
def open_main_window():
    def update_dependencies(*args):
        ...
        dependency_var.set("Keine Abhängigkeit ausgewählt")
    ...
    def update_checkboxes(*args):
        ...
        if selected_dependency != "Keine Abhängigkeit ausgewählt":
    ...
    def generate_plot():
        ...
        if selected_dependency == "Keine Abhängigkeit ausgewählt":
            ...
            ax.set_title(f'Verteilung von "{selected_question}"\nmit "{selected_dependency}"\nin "{", ".join(selected_conditions)}"')
    ...
    main_root = tk.Tk()
    main_root.title("Google Forms-Analyse Tool")
    ...
    dependency_menu = ttk.OptionMenu(right_frame, dependency_var, "Keine Abhängigkeit ausgewählt")
    ...
    generate_button = tk.Button(right_frame, text="Generieren", command=generate_plot)
...
root = TkinterDnD.Tk()
root.title("CSV-Datei auswählen")
...
label = tk.Label(root, text="Ziehen Sie Ihre CSV-Datei hierher", width=40, height=10)
